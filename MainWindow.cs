        ...
        private void btn_auth_Click(object sender, EventArgs e)
        {
            if (tb_auth_password.Text != "")
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    // Но так как мы не можем заранее знать всех свойств JSON объекта мы говорим C#, чтобы он автоматически подстроился под наш JSON/
                    // Для этого мы используем тип данных dynamic (C# пытается автоматически создать поля для объекта data)
